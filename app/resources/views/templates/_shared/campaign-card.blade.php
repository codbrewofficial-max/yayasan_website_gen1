...
<div class="group bg-white rounded-lg shadow overflow-hidden flex flex-col">
    ...
    <div class="p-4 flex-1 flex flex-col">
        <h3 class="font-semibold">
            <a href="{{ route('public.campaign', $campaign->slug) }}" class="group-hover:text-primary">{{ $campaign->title }}</a>
        </h3>
        <p class="mt-1 text-sm text-gray-500 line-clamp-2">{!! \Illuminate\Support\Str::limit(strip_tags($campaign->story ?? ''), 90) !!}</p>

        <div class="mt-4">
            <div class="w-full bg-gray-200 rounded-full h-2">
                <div class="bg-primary h-2 rounded-full" style="width: {{ $campaign->progressPercent() }}%"></div>
            </div>
